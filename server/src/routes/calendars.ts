...
import { campaignCalendarSchema } from "../lib/validation";
 ...
 */
async function canManageCalendar(campaignId: string, userId: string) {
  const campaign = await prisma.campaign.findUnique({
    where: { id: campaignId },
    include: { members: { where: { userId } } },
  });
  if (!campaign) return false;
  if (campaign.dmId === userId) return true;
 ...
 */
async function canViewCalendar(campaignId: string, userId: string) {
  const campaign = await prisma.campaign.findUnique({
    where: { id: campaignId },
    include: { members: { where: { userId } } },
  });
...
router.get("/", async (req, res) => {
  ...
  const calendar = await prisma.campaignCalendar.findUnique({
    where: { campaignId },
    include: {
...
router.post("/", async (req, res) => {
  ...
  const parse = campaignCalendarSchema.safeParse(req.body);
  if (!parse.success) {
    res.status(400).json({ error: parse.error.flatten() });
    return;
  }

  const { name, daysInWeek, weekdayNames, anchorAgeId, anchorMonthId, anchorDay, anchorWeekdayIndex, ages, months, moons } =
    parse.data;
  ...
  try {
    const calendar = await prisma.$transaction(async (tx) => {
      ...
      const ageIdSet = new Set(ages.map((a) => a.id).filter((id): id is string => !!id));
      ...
      // Replace months.
      const monthIdSet = new Set(months.map((m) => m.id).filter((id): id is string => !!id));
      ...
      // Replace moons.
      const moonIdSet = new Set(moons.map((m) => m.id).filter((id): id is string => !!id));
    ...
    const fullCalendar = await prisma.campaignCalendar.findUnique({
